    ...
    public sealed partial class UniversalRenderPipeline : RenderPipeline
    {
        ...
        static void RenderSingleCamera(ScriptableRenderContext context, CameraData cameraData, bool anyPostProcessingEnabled)
        {
            ...
            using (new ProfilingScope(cmdScope, sampler))
            {
                ...
                {
                    ScriptableRenderContext.EmitWorldGeometryForSceneView(camera);// Emit scene view UI
                }
#endif
        ...
        static void RenderCameraStack(ScriptableRenderContext context, Camera baseCamera)
        {
            ...
            bool supportsCameraStacking = renderer != null && renderer.SupportsCameraStackingType(CameraRenderType.Base);//True
            List<Camera> cameraStack = (supportsCameraStacking) ? baseCameraAdditionalData?.cameraStack : null;//是否包含BaseCamera？不包含
            ...
            if (cameraStack != null)
            {
                ...
                for (int i = 0; i < cameraStack.Count; ++i)
                {
                    ...
                    if (currCamera.isActiveAndEnabled)
                    {
                        ...
                        {
                            Debug.LogWarning("Only cameras with compatible renderer types can be stacked. " + $"The camera: {currCamera.name} are using the renderer {currCameraRendererType.Name}, " + $"but the base camera: {baseCamera.name} are using {baseCameraRendererType.Name}. Will skip rendering");
                            continue;
                        }
                        ...
                        {
                            Debug.LogWarning($"Stack can only contain Overlay cameras. The camera: {currCamera.name} " + $"has a type {data.renderType} that is not supported. Will skip rendering.");
                            continue;
                        }
...
#region "XR"
        ...
        static void InitializeCameraData(Camera camera, UniversalAdditionalCameraData additionalCameraData, bool resolveFinalTarget, out CameraData cameraData)
        {
            ...
            cameraData.cameraTargetDescriptor = CreateRenderTextureDescriptor(camera, cameraData.renderScale, cameraData.isHdrEnabled, msaaSamples, needsAlphaChannel, cameraData.requiresOpaqueTexture);
        }
...
#pragma warning restore 0618
        ...
        static void SetupPerFrameShaderConstants()
        {
            ...
            Shader.SetGlobalVector(ShaderPropertyId.glossyEnvironmentColor, glossyEnvColor);//_ENVIRONMENTREFLECTIONS_OFF时使用
